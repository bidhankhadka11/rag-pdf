...
#from langchain_text_splitters import RevursiveCharacterTextSplitter
import tempfile
import shutil
...
def chroma_basics():
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        results = vectorstore.similarity_search(query, k=2) #asking for two results
...
def similarity_search_with_scores():
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        #here the score represent the distance. The close to 0 the more similar
        ...
        for i, (doc, score) in enumerate(results_with_scores):
            print(
                f"Result {i+1}: {doc.page_content} (Score: {score}, Source: {doc.metadata['source']})"
            )
